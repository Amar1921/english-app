...
import {fetchProgress} from "../../store/slices/progressSlice.js";
...
export default function LessonsPage() {
  ...
  const { lessonProgress } = useSelector((s) => s.progress);
  ...
  // Build a quick lookup: lessonId → status
  const progressMap = useMemo(() => {
    const map = {};
    (lessonProgress || []).forEach((p) => { map[p.lessonId] = p.status; });
    return map;
  }, [lessonProgress]);

  const completedIds   = useMemo(() => new Set((lessonProgress || []).filter((p) => p.status === 'COMPLETED').map((p) => p.lessonId)), [lessonProgress]);
  const inProgressIds  = useMemo(() => new Set((lessonProgress || []).filter((p) => p.status === 'IN_PROGRESS').map((p) => p.lessonId)), [lessonProgress]);
      ...
      <Box>
        ...
        <Box sx={{ mb: 4 }}>
          ...
          <Box sx={{
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              {LEVELS.map((lvl) => {
                ...
                const doneCount  = lessonsData.lessons.filter((l) => l.level === lvl && completedIds.has(l.id || l.slug)).length;
                return (
                    <Tooltip key={lvl} title={`${doneCount}/${count} leçons ${lvl} complétées`}>
                      <Box sx={{
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, alignItems: 'flex-start' }}>
            ...
            <ToggleButtonGroup
                value={activeLevel} exclusive
                onChange={(_, v) => v && setActiveLevel(v)}
                size="small" sx={{ flexWrap: 'wrap', gap: 0.25 }}
            >
        ...
        {Object.entries(grouped).map(([lvl, lessons]) => {
          const lvlTotal = lessonsData.lessons.filter((l) => l.level === lvl).length;
          const lvlDone  = lessons.filter((l) => completedIds.has(l.id || l.slug)).length;
          return (
              <Box key={lvl} sx={{ mb: 5 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
                  ...
                  <Typography variant="h6" fontWeight={700} sx={{ letterSpacing: -0.3 }}>
                    {{ A1: 'Débutant', A2: 'Élémentaire', B1: 'Intermédiaire', B2: 'Intermédiaire supérieur', C1: 'Avancé', C2: 'Maîtrise' }[lvl]}
                  </Typography>
                  <Chip
                  ...
                  {lvlDone > 0 && (
                      <Chip
                          ...
                          label={`${lvlDone}/${lvlTotal} faites`}
                          size="small"
                          sx={{ bgcolor: '#22C55E18', color: '#22C55E', fontWeight: 700, fontSize: 11 }}
                      />
                  )}
                </Box>
                ...
                <Box sx={{
                  ...
                  {lessons.map((lesson) => {
                    const lessonKey = lesson.id || lesson.slug;
                    const status = progressMap[lessonKey];
                    return (
                        <LessonCard
                            key={lesson.id}
                            lesson={lesson}
...
function LessonCard({ lesson, levelColor, isDark, status, onClick }) {
  ...
  return (
      <Card
          ...
          sx={{
            border: '1px solid',
            borderColor: isCompleted ? '#22C55E44' : isInProgress ? levelColor + '44' : 'divider',
            borderRadius: 2,
            position: 'relative',
            transition: 'all 0.18s ease',
            '&:hover': {
              borderColor: isCompleted ? '#22C55E88' : levelColor + '88',
              boxShadow: `0 4px 20px ${isCompleted ? '#22C55E' : levelColor}18`,
      ...
      >
        ...
        {isInProgress && !isCompleted && (
            <Box sx={{
              ...
              display: 'flex', alignItems: 'center', gap: 0.4,
              px: 0.9, py: 0.3, borderRadius: 1,
              bgcolor: levelColor, color: 'white',
              fontSize: 10, fontWeight: 800,
            }}>
              <PlayCircleOutlineRounded sx={{ fontSize: 11 }} />
              EN COURS
            </Box>
        )}
        ...
        <CardActionArea onClick={onClick} sx={{ p: 0 }}>
          <CardContent sx={{ p: 2.5 }}>
            {/* Colored top line for completed */}
            {isCompleted && (
                <Box sx={{ height: 3, bgcolor: '#22C55E', borderRadius: 1, mb: 2, mx: -2.5, mt: -2.5 }} />
            )}
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1.5 }}>
              ...
              <Box sx={{
                ...
                mr: status ? 6 : 0,   // leave room for badge
              }}>
                {lesson.level}
              </Box>
            </Box>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Box sx={{ display: 'flex', gap: 1.5 }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.4, fontSize: 12, fontWeight: 600,
                  color: isCompleted ? '#22C55E' : '#F59E0B' }}>
                  <BoltRounded sx={{ fontSize: 13 }} />
                  {isCompleted ? `+${lesson.xpReward} XP ✓` : `+${lesson.xpReward} XP`}
                </Box>
              </Box>
              {!status && (
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.4, color: 'text.disabled', fontSize: 11 }}>
                    <LockOpenRounded sx={{ fontSize: 12 }} />
                    À faire
                  </Box>
              )}
